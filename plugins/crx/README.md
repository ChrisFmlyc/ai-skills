...
- `/crx:loop` — the hands-off wrapper. Sets the outcome with `/goal`, then loops: wait for CodeRabbit's review, pull unresolved findings via `gh`, dispatch them to `/crx:single` or `/crx:multi`, post rebuttal replies and resolve threads for won't-fix findings, push the round's fixes, and repeat until the PR has zero outstanding CodeRabbit threads. Run it under `/loop` (`/loop /crx:loop`) so the waiting between review rounds is paced for you. Invoking it is your standing authorization to push fix commits to that PR branch — unlike the other two skills, which never push.
